...
# Set Output dir
WIDEVINE_INSTALL_DIR="${WIDEVINE_INSTALL_DIR:-$QTDIR/plugins/ppapi/}"
...
if [ $(whoami) = "root" ]; then
  ...
  su -c "sh -c \"install -Dm644 libwidevinecdm.so $WIDEVINE_INSTALL_DIR/libwidevinecdm.so && install -m644 manifest.json $WIDEVINE_INSTALL_DIR/manifest.json && install -m644 LICENSE.txt $WIDEVINE_INSTALL_DIR/LICENSE.txt\""
fi
